...
int parseVersionNumber(const std::string& s){ if(s.size()!=2 || s[0]!='V' || s[1]<'1' || s[1]>'4') throw std::invalid_argument("version must be V1..V4"); return s[1]-'1'; }
void runBenchmark(const std::string& versionText,std::size_t width,std::size_t height,int warmup,int reps,int trials,const std::string& output,bool single){
 const auto version=static_cast<TransposeVersion>(parseVersionNumber(versionText)); const std::size_t n=width*height; std::vector<float> h(n,1.0f), check(n); float *di=nullptr,*do_=nullptr; CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&di),n*sizeof(float))); CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&do_),n*sizeof(float))); CUDA_CHECK(cudaMemcpy(di,h.data(),n*sizeof(float),cudaMemcpyHostToDevice)); launchTranspose(version,di,do_,width,height); CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize()); CUDA_CHECK(cudaMemcpy(check.data(),do_,n*sizeof(float),cudaMemcpyDeviceToHost)); if(std::memcmp(h.data(),check.data(),n*sizeof(float))!=0) throw std::runtime_error("benchmark correctness sanity failed"); if(single){ launchTranspose(version,di,do_,width,height); CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize()); CUDA_CHECK(cudaFree(di)); CUDA_CHECK(cudaFree(do_)); return; } std::ofstream out(output,std::ios::app); if(!out) throw std::runtime_error("cannot open benchmark output"); for(int trial=1;trial<=trials;++trial){ for(int i=0;i<warmup;++i){launchTranspose(version,di,do_,width,height); CUDA_CHECK(cudaGetLastError());} CUDA_CHECK(cudaDeviceSynchronize()); cudaEvent_t st,sp; CUDA_CHECK(cudaEventCreate(&st)); CUDA_CHECK(cudaEventCreate(&sp)); CUDA_CHECK(cudaEventRecord(st)); for(int i=0;i<reps;++i){launchTranspose(version,di,do_,width,height); CUDA_CHECK(cudaGetLastError());} CUDA_CHECK(cudaEventRecord(sp)); CUDA_CHECK(cudaEventSynchronize(sp)); float total=0; CUDA_CHECK(cudaEventElapsedTime(&total,st,sp)); CUDA_CHECK(cudaEventDestroy(st)); CUDA_CHECK(cudaEventDestroy(sp)); double ms=static_cast<double>(total)/reps; double gb=(2.0*static_cast<double>(n)*sizeof(float))/(ms*1.0e6); out<<utcTimestamp()<<','<<versionText<<','<<width<<','<<height<<','<<trial<<','<<warmup<<','<<reps<<','<<ms<<','<<gb<<"\n"; } CUDA_CHECK(cudaFree(di)); CUDA_CHECK(cudaFree(do_)); }
#ifndef EXP03_KERNEL_ONLY
