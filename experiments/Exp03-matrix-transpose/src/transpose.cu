...
Row runCase(TransposeVersion version, Pattern pattern, std::size_t width, std::size_t height, int repeat) {
  Row row{versionName(version),patternName(pattern),width,height,repeat,static_cast<int>((width+TILE_DIM-1)/TILE_DIM),static_cast<int>((height+TILE_DIM-1)/TILE_DIM),TILE_DIM,BLOCK_ROWS,false,false,"PASS",""};
  try {
...
void launchTranspose(TransposeVersion v,const float* in,float* out,std::size_t w,std::size_t h,void* stream){dim3 b(TILE_DIM,BLOCK_ROWS),g((w+TILE_DIM-1)/TILE_DIM,(h+TILE_DIM-1)/TILE_DIM);cudaStream_t s=reinterpret_cast<cudaStream_t>(stream);switch(v){case TransposeVersion::V1Copy:copyKernel<<<g,b,0,s>>>(in,out,w,h);break;case TransposeVersion::V2Naive:naiveKernel<<<g,b,0,s>>>(in,out,w,h);break;case TransposeVersion::V3Tiled:tiledKernel<0><<<g,b,0,s>>>(in,out,w,h);break;case TransposeVersion::V4Padded:tiledKernel<1><<<g,b,0,s>>>(in,out,w,h);break;}}
